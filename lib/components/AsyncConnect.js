...
var AsyncConnect = function (_Component) {
  ...
  function AsyncConnect(props, context) {
    ...
    _this.state = {
      propsToShow: _this.isLoaded(props.filter) ? props : null
    };
  ...
  AsyncConnect.prototype.isLoaded = function isLoaded() {
    var filter = arguments.length <= 0 || arguments[0] === undefined ? function () {
      return true;
    } : arguments[0];

    var componentKeys = this.props.components.reduce(function (componentKeysMemo, component) {
      if (component && component.reduxAsyncConnect && component.reduxAsyncConnect.length) {
        var asyncConnectComponentKeys = component.reduxAsyncConnect.filter(function (asyncConnectComponent) {
          return !!asyncConnectComponent.key && filter(asyncConnectComponent, component);
        }) // eslint-disable-line max-len
        .map(function (asyncConnectComponent) {
          return asyncConnectComponent.key;
        });

        return [].concat(componentKeysMemo, asyncConnectComponentKeys);
      }

      return componentKeysMemo;
    }, []);

    var loadState = this.context.store.getState().reduxAsyncConnect.loadState;
    var reduxAsyncConnectComponentsLoaded = componentKeys.reduce(function (loadedMemo, key) {
      return loadState[key] ? loadedMemo && loadState[key].loaded : false;
    }, true);

    return this.context.store.getState().reduxAsyncConnect.loaded && reduxAsyncConnectComponentsLoaded;
  };
